...
Triangle::Triangle(void) : _vbo(0), _texture(0), _mvp(-1), _mv(-1), _mvit(-1), _l(-1), _lmvp(-1),
...
void Triangle::Render(glm::mat4 view, glm::mat4 projection, glm::mat4 light)
{
    ...
    if (light[3][3] == 0) {
        gl::UniformMatrix4fv(_lmvp, 1, GL_FALSE, glm::value_ptr(modelViewProj));
    } else {
        ...
        glm::mat4 lightMVP = projection * light * model;
        gl::UniformMatrix4fv(_l, 1, GL_FALSE, glm::value_ptr(lightMVP));
    }
    // Bind the VBO
    gl::BindBuffer(GL_ARRAY_BUFFER, _vbo);
...
bool Cube::Init(pvr::Shell *shell, uint32_t* mvpLoc)
{
    ...
    static const float CS = 0.5f; // Cube size
